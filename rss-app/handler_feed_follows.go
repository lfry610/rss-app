...
func (apiCfg *apiConfig) handlerGetFeedFollow(w http.ResponseWriter, r *http.Request) {
	feedFollow, err := apiCfg.DB.GetFeeds(r.Context())
	if err != nil{
		respondWithError(w, 403, fmt.Sprintf("Couldn't get feed follow: %v",err))
		return
	}
	respondWithJSON(w, 201, databaseFeedsToFeeds(feedFollow))
}